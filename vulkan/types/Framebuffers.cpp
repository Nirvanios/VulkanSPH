...
Framebuffers::Framebuffers(std::shared_ptr<Device> device, std::shared_ptr<Swapchain> swapchain,
...
void Framebuffers::createFramebuffers() {
  ...
  for (size_t i = 0; i < swapchain->getSwapChainImageViews().size(); i++) {
    std::array<vk::ImageView, 2> attachments = {swapchain->getSwapChainImageViews()[i].get(),
                                                depthImageView.get()};
