...
bool Instance::checkValidationLayerSupport() {
  ...
  return std::any_of(availableLayers.begin(), availableLayers.end(),
                     [layerNames](const auto &layerProperties) {
...
void Instance::setupDebugMessenger() {
  if (debug) {
    ...
    debugMessenger = instance->createDebugUtilsMessengerEXTUnique(
        createInfo, nullptr, vk::DispatchLoaderDynamic{instance.get(), vkGetInstanceProcAddr});
  }
}
...
const vk::Instance &Instance::getInstance() const { return instance.get(); }
Instance::~Instance() {
  debugMessenger.release();
  instance.release();
}
const std::vector<const char *> &Instance::getValidationLayers() const { return validationLayers; }
