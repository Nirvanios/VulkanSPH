...
void Device::pickPhysicalDevice() {
    ...
    auto it = std::find_if(devices.begin(), devices.end(), [this](const vk::PhysicalDevice &phyDevice) {
    ...
    for (const auto &availableDevice : devices) {
        if (availableDevice == *it) spdlog::info(fmt::format("Available GPU: {}.<-- SELECTED", availableDevice.getProperties().deviceName));
        else
...
Device::QueueFamilyIndices Device::findQueueFamilies(const vk::PhysicalDevice &device, const vk::UniqueSurfaceKHR &surface) {
    ...
    std::for_each(queueFamilies.begin(), queueFamilies.end(), [&i, &indices, &device, &surface](const vk::QueueFamilyProperties &property) {
        auto presentSupport = device.getSurfaceSupportKHR(i, surface.get());
        if (property.queueFlags & vk::QueueFlagBits::eGraphics) indices.graphicsFamily = i;
...
void Device::createLogicalDevice() {
    ...
    for (uint32_t queueFamily : uniqueQueueFamilies) {
        vk::DeviceQueueCreateInfo queueCreateInfo{.queueFamilyIndex = queueFamily, .queueCount = 1, .pQueuePriorities = &priority};
...
vk::Queue Device::getGraphicsQueue() const { return device->getQueue(indices.graphicsFamily.value(), 0); }

vk::Queue Device::getPresentQueue() const { return device->getQueue(indices.presentFamily.value(), 0); }

vk::Queue Device::getComputeQueue() const { return device->getQueue(indices.computeFamily.value(), 0); }
