...
TestRenderer::TestRenderer(const Config &config)
    ...
    vulkanCore.initVulkan(Utilities::loadModelFromObj(config.getApp().simulation.particleModel, glm::vec3{0.5, 0.8, 1.0}), std::span<ParticleRecord>{particles.data(), particles.size()});
...
std::vector<ParticleRecord> TestRenderer::createParticles() {
    ...
    const auto particleSize = glm::vec3(std::cbrt(simConfig.fluidVolume / static_cast<float>(simConfig.particleCount)));
    ...
    for (int z = 0; z < sizeZ; ++z) {
        for (int y = 0; y < sizeY; ++y) {
            for (int x = 0; x < sizeX; ++x) {
                data[(z * sizeY * sizeX) + (y * sizeX) + x].position = glm::vec4{x, y, z, 0.0f} * glm::vec4(particleSize, 0.0f);
