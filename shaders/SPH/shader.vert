...
layout(push_constant) uniform DrawType { int drawType; int visualizationType; }
drawType;
  ...
  switch (drawType.drawType) {
    case DRAW_PARTICLE:
      ...
      switch (drawType.visualizationType) {
        case TEXTURE_VIZUALIZE_NONE:
          fragColor = vec4(inColor, 1.0f);
          break;
        case TEXTURE_VIZUALIZE_PRESSUREFORCE:
          fragColor = vec4(hsv2rgb(vec3(map2hue(particleRecords[gl_InstanceIndex].pressureForceLength, 1300, 700), 1.0, 1.0)), 1.0f);
          break;
        case TEXTURE_VIZUALIZE_MASSDENSITY:
          fragColor = vec4(hsv2rgb(vec3(map2hue(particleRecords[gl_InstanceIndex].massDensity, 1300, 700), 1.0, 1.0)), 1.0f);
          break;
        case TEXTURE_VIZUALIZE_VELOCITY:
          fragColor = vec4(hsv2rgb(vec3(map2hue(length(particleRecords[gl_InstanceIndex].velocity), 1300, 700), 1.0, 1.0)), 1.0f);
          break;
      }
      break;
    case DRAW_OTHER:
