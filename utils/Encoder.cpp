...
Encoder::Encoder()
    : formatContext(
        std::unique_ptr<AVFormatContext, AVFormatContextDeleter>(nullptr, [](auto *) {})),
      packet(std::unique_ptr<AVPacket, AVPacketDeleter>{av_packet_alloc(), [](auto *) {}}),
      outputStream(std::unique_ptr<AVStream, AVStreamDeleter>{nullptr, [](auto *) {
...
void Encoder::initEncoder(int width, int height, unsigned int framerate,
  ...
  outputStream = std::unique_ptr<AVStream, AVStreamDeleter>{
      avformat_new_stream(formatContext.get(), nullptr), [](auto *) {}};
  ...
  codecContext = std::unique_ptr<AVCodecContext, AVCodecContextDeleter>{
      avcodec_alloc_context3(codec.get()), [](auto *) {}};
  ...
  codecContext->time_base =
      outputStream->time_base = {.num = 1, .den = static_cast<int>(framerate)};
