...
const Window &GlfwWindow::getWindow() const { return window; }
void GlfwWindow::cleanUp() {
  spdlog::debug("Destroying window.");
  glfwTerminate();
}
GlfwWindow::~GlfwWindow() { cleanUp(); }
const std::string &GlfwWindow::getWindowName() const { return windowName; }
...
void GlfwWindow::mouseButtonCallback(GLFWwindow *window, int button, int action, int mods) {
  ...
  windowPtr->notifyMouseButton(
      {.button = button, .action = MouseButtonAction{action}, .modifier = Modifier{mods}});
}
void GlfwWindow::keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods) {
  const auto windowPtr = reinterpret_cast<GlfwWindow *>(glfwGetWindowUserPointer(window));

  windowPtr->notifyKey(
      {.key = key, .scancode = scancode, .action = KeyAction{action}, .modifier = Modifier{mods}});
}
